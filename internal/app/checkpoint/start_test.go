...
func TestCountPendingRecommendations(t *testing.T) {
	...
	// Create .checkpoint directory for project file
	checkpointDir := filepath.Join(tmpDir, config.CheckpointDir)
	if err := os.MkdirAll(checkpointDir, 0755); err != nil {
		t.Fatalf("failed to create checkpoint dir: %v", err)
	}

	projectPath := filepath.Join(checkpointDir, config.ExplainProjectYaml)
