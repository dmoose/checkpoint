...
func gatherSummaryData(projectPath, changelogPath string) summaryData {
	...
	projectFilePath := filepath.Join(projectPath, config.CheckpointDir, config.ExplainProjectYaml)
...
func printHumanSummary(data summaryData) {
	...
	if data.pendingRecommendations > 0 {
		fmt.Printf("⚠ %d pending recommendation(s) in .checkpoint/project.yaml\n", data.pendingRecommendations)
	}
	fmt.Println()
	...
	fmt.Println("Tip: Run 'guardrail guide' to view available guides")
	fmt.Println()
}
