...
var configCmd = &cobra.Command{
	Use:   "config",
	Short: "Manage knowledge base configuration files",
	Long: `Read and modify knowledge base configuration files programmatically.
...
var configGetCmd = &cobra.Command{
	...
	Long: `Read any .checkpoint/*.yaml file and output as JSON.

Examples:
  guardrail config get project.yaml
  guardrail config get tools.yaml
  guardrail config get guidelines.yaml`,
...
Examples:
  guardrail config set project.yaml name "My Project"
  guardrail config set tools.yaml build.default.command "make build"
  guardrail config set guidelines.yaml rules[0] "New rule"`,
...
func configList(projectPath string) {
	...
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  guardrail config get <file>              Read as JSON")
	fmt.Println("  guardrail config set <file> <path> <v>   Update value")
}
