...
var initCmd = &cobra.Command{
	Use:   "init [path]",
	Short: "Initialize knowledge base in a project",
...
Auto-detects project language and sets up config files.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		initKnowledgeBase(absPath, Build.Version)
	},
}
...
// initKnowledgeBase creates the knowledge base directory structure and files
func initKnowledgeBase(projectPath string, version string) {
...
These files are referenced by checkpoint and guardrail commands and can be read directly:

- Run ` + "`guardrail examples`" + ` to view examples
- Run ` + "`guardrail guide [topic]`" + ` to view guides
...
`
	...
	if err := InitGlobalSkills(); err != nil {
		fmt.Fprintf(os.Stderr, "warning: could not initialize global skills: %v\n", err)
	}
...
**First time here?** Run ` + "`guardrail guide first-time-user`" + ` for a complete walkthrough.

**Using with LLM?** Run ` + "`guardrail guide llm-workflow`" + ` for integration patterns.

**Need examples?** Run ` + "`guardrail examples`" + ` to see well-structured checkpoints.
...
- .checkpoint-context.yaml: Append-only context log; captures reasoning and decisions
- .checkpoint/project.yaml: Project-wide patterns and conventions (human-curated)
...
7. Periodically: review .checkpoint/project.yaml recommendations and curate
...
- ` + "`guardrail guide first-time-user`" + ` - Complete walkthrough for newcomers
- ` + "`guardrail guide llm-workflow`" + ` - LLM integration patterns and workflow
- ` + "`guardrail guide best-practices`" + ` - Best practices for effective checkpoints

**Examples:**
- ` + "`guardrail examples`" + ` - List all available examples
- ` + "`guardrail examples feature`" + ` - See good feature checkpoint
- ` + "`guardrail examples bugfix`" + ` - See good bug fix checkpoint
- ` + "`guardrail examples anti-patterns`" + ` - Learn what to avoid
...
**For LLMs:**
- Use ` + "`guardrail prompt fill-checkpoint`" + ` to get instructions for filling checkpoint entries
...
- See ` + "`guardrail guide llm-workflow`" + ` for detailed instructions
...
- See ` + "`guardrail guide best-practices`" + ` for more tips

## Commands

**Checkpoint (commit tracking):**
...
- ` + "`guardrail init`" + ` - Initialize knowledge base in project
- ` + "`guardrail explain [topic]`" + ` - Get project context for LLMs
- ` + "`guardrail learn <content>`" + ` - Capture knowledge during development
- ` + "`guardrail skill [action]`" + ` - Manage skills for LLM context
- ` + "`guardrail examples [category]`" + ` - View examples
- ` + "`guardrail guide [topic]`" + ` - View guides
- ` + "`guardrail prompt [id]`" + ` - View LLM prompts
- ` + "`guardrail config`" + ` - Manage configuration files
- ` + "`guardrail doctor`" + ` - Check knowledge base health
- ` + "`guardrail help`" + ` - Show all commands
...
` + "```bash" + `
guardrail prompt                          # List available prompts
guardrail prompt fill-checkpoint          # Get checkpoint fill instructions
guardrail prompt implement-feature \
...
`
	...
	fmt.Printf("\nNext: Run 'guardrail explain' to see project context\n")
