...
func Learn(projectPath string, opts LearnOptions) {
	...
	if opts.Content == "" {
		...
		fmt.Fprintf(os.Stderr, "usage: guardrail learn <content> [flags]\n")
		...
		fmt.Fprintf(os.Stderr, "\nExamples:\n")
		fmt.Fprintf(os.Stderr, "  guardrail learn \"Always validate input at API boundaries\" --guideline\n")
		fmt.Fprintf(os.Stderr, "  guardrail learn \"Don't use global mutable state\" --avoid\n")
		fmt.Fprintf(os.Stderr, "  guardrail learn \"make test-race\" --tool race\n")
		os.Exit(1)
	}
	...
	if _, err := os.Stat(checkpointDir); os.IsNotExist(err) {
		...
		fmt.Fprintf(os.Stderr, "hint: Run 'guardrail init' first\n")
		os.Exit(1)
	}
...
func addTool(checkpointDir, name, command string) error {
	...
	tools.Maintenance[name] = explain.ToolCommand{
		Command: command,
		Notes:   fmt.Sprintf("Added via 'guardrail learn' on %s", time.Now().Format("2006-01-02")),
	}
...
func addLearning(checkpointDir, content string) error {
	// Add to a learnings.yaml file (append-only log)
	...
	fmt.Printf("  (saved to .checkpoint/learnings.yaml)\n")
	return nil
}
...
func listLearnings(projectPath string, jsonOutput bool) {
	learningsPath := filepath.Join(projectPath, config.CheckpointDir, "learnings.yaml")
	...
	if err != nil {
		if os.IsNotExist(err) {
			if jsonOutput {
				...
				fmt.Println("Use 'guardrail learn <content>' to capture learnings.")
			}
			return
		}
