...
func Prompt(projectPath string, promptID string, vars map[string]string, jsonOutput bool) {
	...
	if !file.Exists(promptsDir) {
		...
		fmt.Fprintf(os.Stderr, "Hint: Run 'guardrail init' to create the directory structure\n")
		os.Exit(1)
	}
...
func listPrompts(config *prompts.PromptsConfig, jsonOutput bool) {
	...
	fmt.Println("Usage:")
	fmt.Println("  guardrail prompt <id>")
	fmt.Println("  guardrail prompt <id> --var key=value")
	fmt.Println()
	fmt.Println("Examples:")
	fmt.Println("  guardrail prompt fill-checkpoint")
	fmt.Println("  guardrail prompt implement-feature --var feature_name=\"User Auth\"")
	fmt.Println()
}
...
func showPrompt(config *prompts.PromptsConfig, promptsDir string, projectPath string, promptID string, userVars map[string]string) {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'guardrail prompt' to see available prompts\n")
		os.Exit(1)
	}
